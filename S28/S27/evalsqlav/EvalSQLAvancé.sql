...
Creez une vue qui affiche le catalogue produits. L'id, la reference et le nom des produits,
ainsi que l'id et le nom de la categorie doivent apparaitre.
Pour creer une vue ,on utilise :
...
Creez la procedure stockee facture qui permet d'afficher les informations necessaires à une facture 
en fonction d'un numero de commande. Cette procedure doit sortir le montant total de la commande.
Pour creer une procedure on utilise :
...
/*Presentez le declencheur after_products_update demandee dans la phase 2 de la seance sur les declencheurs.*/
...
Il a ete decide, apres de nombreuses tractations, de confier son poste au pepinieriste 
le plus ancien en poste dans ce magasin. 
Ce dernier voit alors son salaire augmenter de 5% et ses anciens collegues pepinieristes passent sous sa direction.

Ecrire la transaction permettant d'acter tous ces changements en base des donnees.

    La base de donnees ne contient actuellement que des employes en postes. 
    Il a ete choisi de garder en base une liste des anciens collaborateurs de l'entreprise parti en retraite. 
    Il va donc vous falloir ajouter une ligne dans la table posts pour referencer les employes à la retraite.

    /*Decrire les operations qui seront à realiser sur la table posts.*/

    Ecrire les requetes correspondant a� ces operations.

    Ecrire la transaction
 */
  /*Decrire les operations qui seront à realiser sur la table posts.*/
1.inserer une nouvelle donnee dans posts pour retraites
...
WHERE pos_libelle = 'pepinieriste' AND sho_id=2 group by emp_id 
LIMIT 1 ;
