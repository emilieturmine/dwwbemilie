...
--
-- Hôte : localhost:3306
-- Généré le : mer. 07 juil. 2021 à 16:37
...
-- Procédures
--
CREATE DEFINER=`emilie_turmine`@`localhost` PROCEDURE `CA_Supplier` (IN `p_sup_id` INT, IN `p_ord_payment_date` INT, OUT `p_Ca_potentiel` INT)  SELECT SUM(ode_unit_price)INTO p_Ca_potentiel
FROM orders_details
...
CREATE DEFINER=`emilie_turmine`@`localhost` PROCEDURE `Lst_Rush_Orders` (IN `p_ord_status` VARCHAR(25))  Select * from orders
where ord_status = p_ord_status$$

CREATE DEFINER=`emilie_turmine`@`localhost` PROCEDURE `lst_suppliers` ()  select sup_name from suppliers
join products on pro_sup_id= sup_id
join orders_details on ode_pro_id= pro_id
where  ode_quantity is NOT null
group by sup_name$$

CREATE DEFINER=`emilie_turmine`@`localhost` PROCEDURE `verifFrs` (IN `p_sup_zipcode` VARCHAR(5))  BEGIN
...
INSERT INTO `commander_articles` (`codart`, `qte`, `datedujour`) VALUES
(8, 5, '2021-07-07 15:12:54'),
(8, 1, '2021-07-07 16:12:04'),
(8, 1, '2021-07-07 16:14:57');
...
(8, 27, '249.99', 'barb002', NULL, 3, 5, 'Noir', 'Athos', 'Cu\'rabitur pellentesque posuere luctus. Sed et risus vel quam pharetra pretium non quis odio. Praesent varius risus vel orci ultrices tincidunt.', 0, 4, '2016-06-14 00:00:00', NULL, 'jpg'),
...
THEN
INSERT  ignore INTO commander_articles(codart,qte,datedujour) VALUES (New.pro_id,1,Now());
END IF;
END
$$
DELIMITER ;
...
--
ALTER TABLE `posts`
  MODIFY `pos_id` int UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=36;
